...
mopsHub.Functions = {
    ...
    isVisible = function(Position, Ignore) Ignore = Ignore or { Camera } return #Camera:GetPartsObscuringTarget({ Position }, Ignore) == 0 end,
    ...
    Aim_Assist = {
        getTarget = function()
            ...
            for _, plr in pairs(_players) do
                ...
                local h = mopsHub.Misc.Hitbox_Parts[mopsHub.Aim_Assist.Target_Hitbox]
                local _hitbox = _plr_character[h]
...
mopsHub.Functions.spawnTask(function()
	...
	local function TrigEventProxy(event, ...)
		local args = { ... }
		if event == mopsHub.Client.event_enum.FPV_SOL_BULLET_SPAWN then
    ...
    OldTrigEvent = hookfunction(mopsHub.Client.utils.gbus.trig_event, function(...)
...
mopsHub.Connections["_charmods"] = mopsHub.Functions.renderStepped:Connect(function()
    if mopsHub.Client.globals.fpv_sol_multipliers then
        mopsHub.Client.globals.fpv_sol_multipliers.movement = mopsHub.Character_Modifications.WalkSpeedMultiplier.Multiplier
    end
end)
...
local player
mopsHub.Functions.spawnTask(function()
    local slapAHoe = mopsHub.Client.enums.c_net_msg.MELEE_HIT_SOL;
    local cliState = mopsHub.Client.globals.cli_state;
    local combatNetShit = mopsHub.Client.globals.combat_net_msg_state;
    local fpvEquip = mopsHub.Client.globals.fpv_sol_equipment;
    local hitboxHash = mopsHub.Client.globals.soldier_hitbox_hash;
    local netShit = mopsHub.Client.utils.net_msg_util.c_prep_net_msg;
    ...
    local function hit(p1)
        ...
        mopsHub.Client.trig_event(mopsHub.Client.utils.gbus.EVENT_ENUM.FPV_SOL_MELEE_SOL_HIT, equipment, p1, hitboxPos);
    end;
    ...
    mopsHub.Connections["_killAll"] = mopsHub.Functions.renderStepped:Connect(function()
        if mopsHub.KillAll then
